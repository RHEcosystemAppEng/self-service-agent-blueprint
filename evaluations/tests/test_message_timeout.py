...
def test_default_for_chat_script() -> None:
    ...
    assert default_message_timeout_for_script("/app/test/chat-responses-request-mgr.py") == 60


def test_default_for_ticket_script() -> None:
    expected = int(ticket_harness_poll_timeout() + TICKET_HARNESS_POLL_BUFFER_SEC)
    assert (
        default_message_timeout_for_script("ticket-responses-request-mgr.py") == expected
    )


def test_resolve_honors_explicit_timeout() -> None:
    assert (
        resolve_message_timeout("ticket-responses-request-mgr.py", 60)
        == 60
    )


def test_resolve_auto_for_ticket_script(monkeypatch) -> None:
