...
def load_model():
    ...
    logger.info(f"Loading PromptGuard model: {model_id} on device: {device}")
...
def chat_completions(request: dict):
    ...
    # Run inference
    inputs = tokenizer(user_msg, return_tensors="pt", truncation=True, max_length=512).to(device)
    ...
    logger.info(f"Classification: {result} (confidence: {confidence:.4f}) | Message length: {len(user_msg)} chars (original: {original_length})")
    logger.info(f"Message: {user_msg[:100]}{'...' if len(user_msg) > 100 else ''}")

    return {
        "id": "chatcmpl-pg",
        "object": "chat.completion",
        "model": os.getenv("PROMPTGUARD_MODEL_ID", "meta-llama/Llama-Prompt-Guard-2-86M"),
